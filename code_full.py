...
class ConvLayer:
    def __init__(self, in_channel, out_channel, kernel_size, lr=0.01, momentum=0.9, name='Conv'):
        '''
            in_channel: số lượng input
            out_channel: số lượng output
            kenel_size: kích thước kernel
            learing_rate: tốc độ học
            momentum:
        '''
    ...
    def backward(self, residual):
        ...
        # gradient_b
        self.gradient_b = residual.sum(axis=3).sum(axis=2).sum(axis=0) / self.batch_size #Tổng tất cả các pixel trong ma trận lỗi chia cho số ma trận
        # gradient_w
...
if __name__ == '__main__':
    ...
    print('net build ok')
    net.train(train_feature, train_label, valid_feature, valid_label, 16, 10)
